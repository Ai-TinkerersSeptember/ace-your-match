...
const generateCSVFiles = () => {
  console.log('🚀 Starting to generate 300 user profiles as CSV files...');
  ...
  console.log('Generated all profiles. Now creating CSV files...');
  ...
  const profilesHeader = 'id,name,age,gender,bio,location,latitude,longitude,profile_photo_url,created_at,updated_at\n';
  const profilesRows = allProfiles.map(p => 
    `${p.id},${escapeCSV(p.name)},${p.age},${p.gender},${escapeCSV(p.bio)},${escapeCSV(p.location)},${p.latitude},${p.longitude},${p.profile_photo_url},${p.created_at},${p.updated_at}`
  ).join('\n');
  fs.writeFileSync(path.join(outputDir, 'profiles.csv'), profilesHeader + profilesRows);
  
  // Generate user_preferences CSV
  console.log('Creating user_preferences.csv...');
  const preferencesHeader = 'id,user_id,age_range_min,age_range_max,gender_preference,max_travel_distance,frequency,preferred_days,preferred_time_slots,venue_types,created_at,updated_at\n';
  const preferencesRows = allPreferences.map(p => 
    `${p.id},${p.user_id},${p.age_range_min},${p.age_range_max},${arrayToCSV(p.gender_preference)},${p.max_travel_distance},${p.frequency},${arrayToCSV(p.preferred_days)},${arrayToCSV(p.preferred_time_slots)},${arrayToCSV(p.venue_types)},${p.created_at},${p.updated_at}`
  ).join('\n');
  fs.writeFileSync(path.join(outputDir, 'user_preferences.csv'), preferencesHeader + preferencesRows);
  
  // Generate user_sports CSV
  console.log('Creating user_sports.csv...');
  const sportsHeader = 'id,user_id,sport,skill_level,created_at\n';
  const sportsRows = allSports.map(s => 
    `${s.id},${s.user_id},${s.sport},${s.skill_level},${s.created_at}`
  ).join('\n');
  fs.writeFileSync(path.join(outputDir, 'user_sports.csv'), sportsHeader + sportsRows);
  ...
  - profiles.csv (${allProfiles.length} rows)
  - user_preferences.csv (${allPreferences.length} rows)
  - user_sports.csv (${allSports.length} rows)
  `);
