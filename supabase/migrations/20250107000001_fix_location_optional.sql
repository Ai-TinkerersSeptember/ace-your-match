-- Fix the location optional migration by properly handling function replacement
-- This addresses the "function name is not unique" error
...
-- Make location fields in profiles nullable (they should already be, but ensuring it)
ALTER TABLE profiles 
ALTER COLUMN latitude DROP NOT NULL,
ALTER COLUMN longitude DROP NOT NULL,
ALTER COLUMN location DROP NOT NULL;

-- Drop all existing versions of get_potential_matches function
DROP FUNCTION IF EXISTS public.get_potential_matches(double precision, double precision, integer, integer);
DROP FUNCTION IF EXISTS public.get_potential_matches(double precision, double precision, integer);
DROP FUNCTION IF EXISTS public.get_potential_matches(integer, integer);
DROP FUNCTION IF EXISTS public.get_potential_matches();

-- Create the new version of get_potential_matches with location preferences
...
-- Success message
SELECT 'Location optional feature enabled successfully!' as status;
